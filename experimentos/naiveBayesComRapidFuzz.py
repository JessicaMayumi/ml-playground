...
class ClassificarTransacao: 
    ...
    def _adicionar_prefixo_sufixo(self, texto):
        ...
        numero_aleatorio = str(random.randint(0, 99999999999999))
    ...
    def gerarDataset(self, tamanho = 9999999): 
        df = self.lerDataset()
        if df.empty:
    ...
    def classificarRapidFuzz(self, transacao):
        df = self.lerDataset()
        if df.empty:
            print("\033[31m\033[1mNenhum dado no dataset.\033[0;0m")
            return None, 0.0
        transacoes = df["transacao"].tolist()
        categorias = dict(zip(df["transacao"], df["categoria"]))

        melhor_match, score, _ = process.extractOne(transacao, transacoes, scorer=fuzz.partial_ratio)
        categoria = categorias.get(melhor_match, "outros")

        return categoria, score / 100.0  # converte para escala de 0 a 1
...
def main():
    ...
    while True:
        ...
        print("\033[35m\033[1m4. Comparar modelo vs RapidFuzz\033[0m")
        print("\033[35m\033[1m5. Sair\033[0m")
        ...
        if escolha == "1":
            ...
            texto = input("\033[35m\033[1mDigite o texto da transação para comparar: \033[0;0m\033[35m")
            if classificador.pipeline is None:
                print("\033[31m\033[1mTreine o modelo primeiro para comparar.\033[0;0m")
            else:
                pred_ml, conf_ml = classificador.classificarCategoria(texto)
                pred_rf, conf_rf = classificador.classificarRapidFuzz(texto)

                print("\n\033[36m\033[1m★ Resultado da comparação:\033[0;0m")
                print(f"\033[34mModelo ML     → Categoria: {pred_ml}, Confiança: {conf_ml:.2f}\033[0m")
                print(f"\033[33mRapidFuzz     → Categoria: {pred_rf}, Similaridade: {conf_rf:.2f}\033[0m")
