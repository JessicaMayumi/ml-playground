...
class FinancialClassifier:
    ...
    def load_classifier_if_exists(self):
        """Carrega o modelo, vectorizer e encoder se existirem"""
        try:
    ...
    def add_transaction(self, transaction, category):
        """Adiciona uma nova transação ao dataset"""
    ...
    def augment_text(self, text):
        """Aplica técnicas simples de aumento de texto"""
        techniques = [
            self._shuffle_chars,
    ...
    def _shuffle_chars(self, text):
        """Embaralha alguns caracteres no texto"""
        if len(text) <= 3:
            return text
    ...
    def _change_case(self, text):
        """Altera maiúsculas/minúsculas de parte do texto"""
        if not text:
            return text
    ...
    def balance_dataset(self, df, samples_per_category=None):
        """Balanceia o dataset para ter o mesmo número de amostras por categoria"""
    ...
    def train_model(self):
        """Treina o modelo de rede neural com PyTorch"""
        df = self.load_data()
    ...
    def classify_transaction(self, transaction):
        """Classifica uma transação usando o modelo treinado"""
        if self.model is None or self.vectorizer is None or self.label_encoder is None:
            print("Modelo não treinado. Execute a opção 2 primeiro.")
            return None
        
        # Vetorizar a transação
        ...
        probability = prediction[category_idx]
