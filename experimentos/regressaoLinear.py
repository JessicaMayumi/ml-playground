...
class ClassificarTransacao: 
    ...
    def _adicionar_prefixo_sufixo(self, texto):
        ...
        numero_aleatorio = str(random.randint(0, 99999))
        
        if random.choice([True, False]):
            return random.choice(prefixos) + texto
        else:
            sufixo_escolhido = random.choice(sufixos)
            return texto + sufixo_escolhido + numero_aleatorio
    ...
    def treinaModelo(self, datasetBalenceado = None):
        ...
        self.pipeline = Pipeline([ ('vetorizador', TfidfVectorizer(ngram_range=(1, 2), min_df=2, max_df=0.9)), ('classificador', LogisticRegression(max_iter=1000))]) 
