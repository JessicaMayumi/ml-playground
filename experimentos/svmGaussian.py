...
class ClassificarTransacao: 
    ...
    def gerarDataset(self, tamanho = 999999): 
        df = self.lerDataset()
        if df.empty:
    ...
    def treinaModelo(self, datasetBalenceado = None):
        ...
        self.pipeline = Pipeline([('vetorizador', TfidfVectorizer(ngram_range=(1, 2), min_df=1, max_df=1.0)), ('classificador', SVC(kernel='rbf', probability=True))]) 
